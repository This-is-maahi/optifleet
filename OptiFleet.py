...
# # OptiFleet: Real-Time Fleet Optimization using Kruskal's MST Algorithm
# # Author: Kothuru Sai Mahidar
# # Date: [Insert Date]
# # Purpose: Optimize urban transportation routes in high-demand areas of Bangalore, based on real-time passenger demand.

# class UnionFind:
#     """
#     Union-Find Data Structure for managing disjoint sets in Kruskal's MST algorithm.
#     Keeps track of connected components to avoid cycles in the graph.
#     """
#     def __init__(self, n):
#         self.parent = list(range(n))
#         self.rank = [0] * n
...
# class KruskalMST:
#     """
#     Implements Kruskal's Minimum Spanning Tree (MST) algorithm for route optimization.
#     Uses edges with weights that represent passenger demand to create an efficient spanning tree.
#     """
#     def __init__(self, vertices):
#         self.V = vertices  # Total number of vertices (locations)
#         self.edges = []  # Store edges as (start, end, weight) tuples
...
# class OptiFleet:
#     """
#     Core class for OptiFleet project. Handles real-time data updates and route optimization.
#     """
#     def __init__(self, vertices):
#         self.vertices = vertices
#         self.mst_solver = KruskalMST(vertices)
...
#     def update_passenger_data(self, start, end, volume):
#         """ Update passenger volume between locations and add as edge in MST graph. """
#         self.data_manager.update_passenger_volume(start, end, volume)
#         self.mst_solver.add_edge(start, end, volume)

#     def build_routes(self):
#         """ Build optimal routes using MST based on current passenger data. """
#         edges = self.data_manager.get_edges_with_demand()
#         for start, end, volume in edges:
#             self.mst_solver.add_edge(start, end, volume)

#         mst_routes = self.mst_solver.find_mst()
#         self.visualize_routes(mst_routes)  # Call to visualize the routes
#         return mst_routes

#     def visualize_routes(self, mst_routes):
#         """ Visualize the optimized routes using NetworkX and Matplotlib. """
#         G = nx.Graph()
